...
Cuối phiên thành công: lưu `storage_state.json` + merge `auth_config.json` vào `UPWORK_AUTH_DIR`
(mặc định: `<repo>/.auth/`) để GraphQL đọc — giống thư mục `debug_upwork_graphql/.auth`.

Cần FlareSolverr + Playwright (chromium). Khuyến nghị: `UPWORK_LOGIN_FORM=1` (đăng nhập bằng form).
...
def _save_auth_storage(context: Any, dbg: LoginDebugLog, flaresolverr_url: str) -> None:
    ...
    print(f"[auth] Đã lưu phiên (GraphQL dùng file này): {out}", flush=True)
...
def _login_via_playwright_form(
    ...
    while (time.time() - t0) * 1000 < form_timeout:
        ...
        try:
            # Nếu đã chuyển trạng thái nhưng chưa kịp thay đổi rõ ràng, vẫn thử success_glob nhanh.
