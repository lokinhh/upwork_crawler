#!/usr/bin/env python3
"""
Test logging into Upwork via FlareSolverr + Playwright (real Chromium, do not use curl_cffi/requests to Upwork).
...
2b) Extract **iovation** / **forterToken** from page (script + meta); env override — if still empty, paste UPWORK_IOVATION from DevTools (POST login password step)
3) Login: default **fetch** (2-step API); or **UPWORK_LOGIN_FORM=1** — fill out the form on the page (like a real user, iovation sends a real request)
4) Options: GET subordinate JS, POST visitor-gql-token

End of successful session: save `storage_state.json` + merge `auth_config.json` into `UPWORK_AUTH_DIR`
(default: `<repo>/.auth/`) for GraphQL to read — same as the `debug_upwork_graphql/.auth` directory.

Needs FlareSolverr + Playwright (chromium). Recommended: `UPWORK_LOGIN_FORM=1` (login with form).

Required (env):
  UPWORK_EMAIL
  UPWORK_PASSWORD

File (optional): login.env — copied from login.env.example
  UPWORK_LOGIN_ENV — another env file path

Options:
  UPWORK_PLAYWRIGHT_HEADLESS — default 1; 0 = open window (debug)
  UPWORK_PLAYWRIGHT_VIEWPORT — default 1280x900 (WxH)
  UPWORK_PLAYWRIGHT_GOTO_WAIT — domcontentloaded | load | networkidle (default networkidle; can change if waiting too long)
  ...
  UPWORK_LOGIN_FORM — 1 = login using fill/click form (not using fetch API)
  UPWORK_LOGIN_FORM_TIMEOUT_MS — default 90000
  UPWORK_LOGIN_FORM_SUCCESS_GLOB — default **/nx/** (Playwright glob after login)

Debug: UPWORK_LOGIN_DEBUG, UPWORK_LOGIN_DEBUG_LOG (as before)
Console: UPWORK_LOGIN_VERBOSE — default 1 (print status URL, summary of each step); 0 = main line [n] only

Exit: 0 OK | 1 missing env | 2 FlareSolverr error | 3 failed logins | 4 HTTP/other errors
"""
...
def _login_response_summary(d: Any) -> Dict[str, Any]:
    """JSON summary of login API response (no password)."""
...
def _print_login_failure_hint(d: Any, step: str) -> None:
    if not isinstance(d, dict):
        print(f"[{step}] Response is not a JSON object: {type(d).__name__}", file=sys.stderr)
        return
    print(
...
def _write_fetch_dump(
    ...
    if "challenge" in low and "html" in ct.lower():
        fp.write(
            "\n(note) Challenge HTML page appears — cf_clearance cookie must match UA/session.\n"
        )
...
class LoginDebugLog:
    """Write detailed log to file. path=None → no log (no-op)."""
...
def _load_login_env_file() -> None:
    ...
    if not path.is_file():
        return
    print(f"[0] Read environment variable from {path.name}", flush=True)
...
def _flaresolver_to_playwright_cookies(fs_cookies: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Pass FlareSolverr cookies → format context.add_cookies."""
...
def _auth_dir_path() -> Path:
    env = (os.environ.get("UPWORK_AUTH_DIR") or "").strip()
    if env:
        return Path(env).expanduser().resolve()
    return Path(__file__).resolve().parent.parent.parent / ".auth"


def _save_auth_storage(context: Any, dbg: LoginDebugLog, flaresolverr_url: str) -> None:
    auth = _auth_dir_path()
    auth.mkdir(parents=True, exist_ok=True)
    out = auth / "storage_state.json"
    context.storage_state(path=str(out))
    dbg.line(f"storage_state saved -> {out}")
    print(f"[auth] Session saved (GraphQL uses this file): {out}", flush=True)
    cfg_p = auth / "auth_config.json"
    data: Dict[str, Any] = {}
    if cfg_p.is_file():
        try:
            ...
            data = {}
    data.setdefault("flaresolverr_url", flaresolverr_url.rstrip("/"))
    warm = (os.environ.get("UPWORK_WARM_URL") or "").strip() or (
        "https://www.upwork.com/nx/search/jobs/?q=spring%20boot&page=1"
    )
    data.setdefault("warm_url", warm)
    cfg_p.write_text(json.dumps(data, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    dbg.line(f"merged auth_config.json -> {cfg_p}")


def _load_flaresolverr_url() -> str:
    p = _auth_dir_path() / "auth_config.json"
    if p.is_file():
        try:
...
def _csrf_token(page: Any, context: Any) -> str:
    """CSRF: prioritize cookie jar; fallback document.cookie (browser-like)."""
...
def _attach_login_post_sniffer(page: Any, dbg: LoginDebugLog) -> None:
    """Force POST /ab/account-security/login to log mode + iovation (debug) length."""
...
def _login_via_playwright_form(
    ...
    """Fill in email → submit → wait for password box → fill in → submit; wait for URL to succeed."""
    ...
    for sel in username_selectors:
        try:
            ...
            dbg.line(f"form: ignore selector {sel!r}: {exc}")
            continue
    if not user_filled:
        print("No username box found (try changing selector or UPWORK_PLAYWRIGHT_HEADLESS=0).", file=sys.stderr)
        raise SystemExit(3)
    ...
    def _click_submit(step: str) -> bool:
        for sel in submit_selectors:
            try:
                loc = page.locator(sel).first
                if loc.count() < 1:
                    dbg.line(f"form: {step} — selector {sel!r} not found")
                    continue
                if not loc.is_visible():
                    dbg.line(f"form: {step} — selector not visible {sel!r}")
                    continue
                loc.click(timeout=10_000)
                dbg.line(f"form: {step} — click {sel!r}")
                return True
            except Exception as exc:
                dbg.line(f"form: {step} — unclickable {sel!r}: {exc}")
        return False

    clicked1 = _click_submit("step 1")
    if not clicked1:
        page.keyboard.press("Enter")
        dbg.line("form: step 1 — send Enter")
    ...
    try:
        ...
        dbg.line(f"form: password box not found: {exc}")
        print("No password box after email step — CAPTCHA or selector changed.", file=sys.stderr)
    ...
    for sel in pwd_selectors:
        try:
            ...
            dbg.line(f"form: password try {sel!r}: {exc}")
            continue
    if not pwd_filled:
        print("Unable to fill in the password field.", file=sys.stderr)
        raise SystemExit(3)

    clicked2 = _click_submit("step 2")
    if not clicked2:
        page.keyboard.press("Enter")
        dbg.line("form: step 2 — send Enter")

    try:
        page.wait_for_load_state("domcontentloaded", timeout=15_000)
    except Exception as exc:
        dbg.line(f"form: wait_for_load_state(domcontentloaded) error (ignored): {exc}")

    # Wait for multiple states, avoid blind timeouts if Upwork changes navigation flow.
    t0 = time.time()
    blocker_reason = ""
    blocker_selectors = (
    ...
    while (time.time() - t0) * 1000 < form_timeout:
        ...
        if "/ab/account-security/login" not in cur and "login" not in cur:
            dbg.line(f"form: success — left login page, url={page.url}")
            break
        if any(tok in cur for tok in success_url_tokens):
            dbg.line(f"form: success — URL match token, url={page.url}")
            break

        try:
            # If the status has been changed but not changed clearly, still try success_glob quickly.
            page.wait_for_url(success_glob, timeout=500)
            dbg.line(f"form: success — match success_glob={success_glob!r}, url={page.url}")
            break
        except Exception:
            pass
        ...
        for reason, sel in blocker_selectors:
            try:
                if page.locator(sel).first.is_visible():
                    ...
                    dbg.line(f"form: detected intermediate/error state: {reason} ({sel})")
                    break
            except Exception:
                continue
        if blocker_reason:
        ...
        dbg.line(f"form: timeout waiting for page transition, current_url={page.url}")
        blocker_reason = blocker_reason or "timeout_no_state_change"

    if blocker_reason:
        ts = time.strftime("%Y%m%d_%H%M%S")
        log_dir = Path(os.environ.get("UPWORK_LOG_DIR") or os.environ.get("LOG_DIR") or str(ROOT)).expanduser().resolve()
        log_dir.mkdir(parents=True, exist_ok=True)
        shot = log_dir / f"login_form_stuck_{ts}.png"
        html = log_dir / f"login_form_stuck_{ts}.html"
        try:
            page.screenshot(path=str(shot), full_page=True)
            dbg.line(f"form: screenshot taken: {shot}")
        except Exception as exc:
            dbg.line(f"form: screenshot error: {exc}")
        try:
            html.write_text(page.content(), encoding="utf-8")
            dbg.line(f"form: saved HTML: {html}")
        except Exception as exc:
            dbg.line(f"form: save HTML error: {exc}")
        print(
            f"Login form not completed ({blocker_reason}) — Current URL: {page.url}."
            f"Snapshot saved for debugging.",
            file=sys.stderr,
        )
        raise SystemExit(3)

    dbg.line(f"form: last url={page.url}")
    return {
        "success": 1,
        "userUid": None,
...
def run() -> None:
    ...
    if not email or not password:
        print(
            "UPWORK_EMAIL or UPWORK_PASSWORD is missing in the environment.",
            file=sys.stderr,
        )
        raise SystemExit(1)
    ...
    if log_path:
        print(f"[log] Debug details: {log_path}", flush=True)
    if _verbose_login():
        print(
            "[log] UPWORK_LOGIN_VERBOSE — prints more: HTTP, url, body_len, JSON login summary per step",
            flush=True,
        )
...
def _run_login_flow(email: str, password: str, dbg: LoginDebugLog) -> None:
    ...
    try:
        ...
        print("FlareSolverr returned non-JSON", file=sys.stderr)
        raise SystemExit(2)
    ...
    print(
        f"[2] Received {len(fs_cookies)} cookies from FlareSolverr; Playwright + UA FlareSolverr.",
        flush=True,
    )
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if r0 is None:
                dbg.line("page.goto returned None (navigation error?)")
                print("Playwright: page.goto failed", file=sys.stderr)
                raise SystemExit(4)
            ...
            if r0.status >= 400:
                ...
                if dbg.path:
                    print(f"See details: {dbg.path}", file=sys.stderr)
                raise SystemExit(4)
            ...
            if not csrf:
                ...
                print(
                    "XSRF-TOKEN not found after goto — try UPWORK_PLAYWRIGHT_HEADLESS=0 or check cookies.",
                    file=sys.stderr,
                )
                if dbg.path:
                    print(f"See details: {dbg.path}", file=sys.stderr)
            ...
            print("[2b] Get iovation / forterToken from page (add if not already in env) …", flush=True)
            dbg.section("iovation / forter — excerpt from page")
            try:
                ...
                dbg.line(f"extract token from error page: {exc}")
                iovation_page, forter_page = "", ""
            iovation = iovation_env or iovation_page
            forter = forter_env or forter_page
            dbg.line(
                f"iovation: env len={len(iovation_env)} page len={len(iovation_page)} → use len={len(iovation)}"
            )
            dbg.line(
                f"forter: env len={len(forter_env)} page len={len(forter_page)} → use len={len(forter)}"
            )
            if not iovation:
                dbg.line(
                    "(warning) No iovation after env+page — password step (fetch) usually returns wrongPassword."
                )
                if not _use_form_login():
                    print(
                        "⚠️ No iovation (UPWORK_IOVATION and page are both blank)."
                        "With fetch API login may fail — try UPWORK_LOGIN_FORM=1.",
                        file=sys.stderr,
                    )
            if not forter:
                dbg.line("(hint) empty forterToken — some accounts still need UPWORK_FORTER_TOKEN.")

            use_form = _use_form_login()
            d2: Dict[str, Any]
            if use_form:
                print("[form] UPWORK_LOGIN_FORM=1 — fill out form + click (sniff POST login in log) …", flush=True)
                _attach_login_post_sniffer(page, dbg)
                d2 = _login_via_playwright_form(page, email, password, dbg)
                dbg.section("Successful login (form UI)")
                dbg.line(f"final_url={page.url}")
                print("✓ Login (form).", flush=True)
            if not use_form:
                ...
                if int(tr.get("status") or 0) >= 400:
                    dbg.line(f"GET timestamp failed: HTTP {tr.get('status')}")
                ...
                try:
                    ...
                    if isinstance(_ts1, dict) and "timestamp" in _ts1:
                        dbg.line(f"GET timestamp (1st time) server_ms={_ts1.get('timestamp')}")
                        _log_v(f"    server timestamp (ms): {_ts1.get('timestamp')}")
                except Exception as exc:
                    dbg.line(f"GET timestamp (first time) did not parse JSON: {exc}")
                ...
                if st1 != 200:
                    dbg.line(f"POST login (username) HTTP != 200: body first={(s1.get('text') or '')[:500]!r}")
                ...
                if not isinstance(d1, dict):
                    print("Step 1 login response is not a JSON object", file=sys.stderr)
                    raise SystemExit(3)
                dbg.login_api_summary("Step 1 (username) — parse response", d1)
                print(f"    response keys: {list(d1.keys())}", flush=True)
                _log_v(
                    f" step 1: success={d1.get('success')!r} mode={d1.get('mode')!r} "
                    f"eventCode={d1.get('eventCode')!r} userNid={d1.get('userNid')!r}"
                )
                if d1.get("success") != 0 or d1.get("mode") != "password":
                    dbg.login_api_summary("Step 1 FAILED — details", d1)
                    _print_login_failure_hint(d1, "step 1 (username)")
                    print(json.dumps(d1, indent=2, ensure_ascii=False), file=sys.stderr)
                    raise SystemExit(3)
                user_nid = d1.get("userNid")
                if not user_nid:
                    dbg.login_api_summary("Step 1 — missing userNid", d1)
                    print("Missing userNid", file=sys.stderr)
                    raise SystemExit(3)
                ...
                dbg.fetch_result("GET timestamp (before password)", p2)
                _fetch_console_line("GET timestamp (before password)", p2)
                ...
                if st2 == 200:
                    try:
                        ...
                        dbg.line(f"GET timestamp (before password) JSON: {e}")
                ...
                if st2 == 200 and isinstance(tr2j, dict) and "timestamp" in tr2j:
                    ...
                    dbg.line(
                        "GET timestamp (before password): no timestamp field — use client time as server_ts"
                    )
                ...
                if elapsed_ms < 0:
                    dbg.line(
                        f"elapsedTime negative ({elapsed_ms} ms): clamp 0 — server_ts={server_ts} t1={t1} (clock skew?)"
                    )
                    print(
                        "⚠️ negative elapsedTime — sent 0 (server_ts > t1; usually due to clock skew)",
                        file=sys.stderr,
                    )
                    elapsed_ms = 0
    
                dbg.line(
                    f"prepare password: t1_ms={t1} server_ts_ms={server_ts} elapsedTime_ms={elapsed_ms}"
                )
                _log_v(f" elapsedTime (ms) posted: {elapsed_ms} (t1={t1}, server_ts={server_ts})")
                ...
                if st2p != 200:
                    dbg.line(f"POST login (password) HTTP != 200: first body={(s2.get('text') or '')[:500]!r}")
                ...
                if not isinstance(d2, dict):
                    print("Step 2 login response is not a JSON object", file=sys.stderr)
                    raise SystemExit(3)
                dbg.login_api_summary("Step 2 (password) — parse response", d2)
                _log_v(
                    f" step 2: success={d2.get('success')!r} eventCode={d2.get('eventCode')!r} "
                    f"userUid={d2.get('userUid')!r}"
                )
                print(json.dumps(d2, indent=2, ensure_ascii=False)[:4000], flush=True)
                if d2.get("success") != 1:
                    dbg.login_api_summary("Step 2 FAILED — details", d2)
                    _print_login_failure_hint(d2, "step 2 (password)")
                    raise SystemExit(3)
    
                dbg.section("Successful login")
                dbg.line(f"userUid={d2.get('userUid')} redirectUrl={(d2.get('redirectUrl') or '')[:200]}")
                print("✓ Login: success=1.", flush=True)
                ...
                try:
                    ...
                    if sst == 200:
                        ...
                        if m:
                            ...
                            print(" (does not match pattern token in body JS)", file=sys.stderr)
                except Exception as exc:
                    dbg.line(f"GET subordinate via fetch failed (usually due to CORS), try page.request: {exc}")
                    sr = page.request.get(
                        sub_url,
                        headers={
            ...
            _save_auth_storage(context, dbg, fs)

            print("\n--- Summary ---", flush=True)
            print(f"userUid: {d2.get('userUid')}", flush=True)
            if visitor_token:
                print("visitor-gql accessToken: taken", flush=True)
            if sub_token:
                print("subordinate oauth2v2_int (in JS): taken", flush=True)
        finally:
            browser.close()
...
if __name__ == "__main__":
    try:
        ...
        print(f"HTTP error (requests/FlareSolverr): {exc}", file=sys.stderr)
        raise SystemExit(4)
    except Exception as exc:
        print(f"Error (Playwright / network): {exc}", file=sys.stderr)
        raise SystemExit(4)
